...
public class Copy_2_of_HsgTools
{
	...
	public String overtxt (String txt){
		if (txt!=null && !txt.trim().equals("")){
			ArrayList<String> english = tools.getByREX(txt, "\\w{1}((　)|( )){1,}\\w{1}");
			if (english!=null)  //处理英文间空格问题
			for (int j = 0; j < english.size(); j++)
			{
				String Eng = english.get(j).replaceAll("\\s+", "%0%");
				String eng1 = tools.getStrByREX(Eng, "\\w{1}");
				String eng2 = Eng.replace(eng1+"%0%", "");
				txt = txt.replaceFirst(eng1+"((　)|( ))"+eng2, Eng);
			}
			txt = txt.replaceAll("((◉)|(☉)|(⊙)|(◎)|(＊)|(￢)|(¬)|(◤)|(◥)|(◢)|(◣)|(░)|(▒)|(▓)|(☍)|(☋)|(☌)|(↓)" +
					"|(↑)|(↔)|(®)|(©)|(※)|(●)|(■)|(□)|(☆)|(△)|(▽)|(▼)|(★)|(\\*)|(◇)|(◆)|(>{2,}))", "")
					.replace("&nbsp;","").replace("&amp;","&")
			...
			String duan="";
			ArrayList<String> duan1 = tools.getByREX(txt, "<br\\s*/>\\s*\\d{1,}.*?<br\\s*/>");
			if(duan1!=null&&duan1.size()!=0){
				for (int j = 0; j < duan1.size(); j++)
				{
					duan = duan1.get(j).trim();
					String dd = "\n    "+tools.Html2Text(duan)+"\n    ";
					txt = txt.replace(duan, dd);
				}
			}
			String zhang="";
...
//			ArrayList<String> zhang1 = tools.getByREX(txt, "<br/>\\[.+?\\].{0,20}?<br/>");
			ArrayList<String> zhang1 = tools.getByREX(txt, "<br/>\\d{1,2}．.{0,20}?<br/>");
			String zone = tools.getStrByREX(txt.substring(0, 40), "((\\[.+?\\])|(第[^第]{1,3}((章)|(卷)|(节)))|(\\d{1,2}．)).{0,20}?<br/>");
			...
			if(zhang1!=null&&zhang1.size()!=0){
				for (int j = 0; j < zhang1.size(); j++)
				{
					...
					if(jie!=null&&!jie.trim().equals("")){
						zz = "\n【节】"+tools.Html2Text(zhang)+"\n    ";
						zz = zz.replace(jie, jie+"    ");
					}else if(juan!=null&&!juan.trim().equals("")){
						zz = "\n【章】"+tools.Html2Text(zhang)+"\n    ";
						zz = zz.replace(juan, juan+"    ");
					}else if(zhz!=null&&!zhz.trim().equals("")){
						zz = "\n【章】"+tools.Html2Text(zhang)+"\n    ";
						zz = zz.replace(zhz, zhz+"  ");
...
//			txt = txt.replaceAll("(\\s*((<br>)|(<BR>)|(<br\\s*/>)|(</h4>))\\s*){1,}", "\n    ");
	...
	public  boolean rexName (ArrayList<String> al,String bookUrl){
		boolean result=true;
		String []notName={"宣传","广告","专家","编辑","编者","介绍","名家","读者","本书","媒体","买点","推荐","目录","内容","简介","插图","图）"}; 
