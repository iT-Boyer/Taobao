...
public class 完善简介 {
	...
	public static void main(String[] args) throws IOException {
		...
		List<Books_Info> als = dbd.query(Books_Info.class, Cnd.where("内容提要", "is", null), null);
		...
		for (int a = 0; a < als.size(); a++) {
				...
				//----------------------搜索引擎的返回信息的采集-----------------------
					...
					if(abb!=null){
						String jjString = abb.getJianjie().trim();
						if(jjString!=null&&!jjString.equals("")){
							jjString = (new HsgTools()).overtxt(jjString);
							dbd.update(Books_Info.class,Chain.make("内容提要", abb.getJianjie()),Cnd.where("书名","=",bookName));
							dbd.update(Books_Info.class,Chain.make("版本说明", "当当："+abb.getName()),Cnd.where("书名","=",bookName));
